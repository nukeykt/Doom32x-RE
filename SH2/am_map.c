...
void AM_MarkRect (int x, int y, int width, int height);
...
void AM_Stop (void)
{
  ...
  /*AM_unloadPics()*/;
  automapactive = false;
  players[playernum].automapflags &= ~AF_ACTIVE;
  /*ST_Responder(&st_notify)*/;
  stopped = true;
...
void AM_Control (player_t *player)
{
    ...
    if (!automapactive)
    {
        if ((buttons & DAT_060010e0) != 0 && (obuttons & DAT_060010e0) == 0)
        {
            AM_Start();
            viewactive = true;
            /*      viewactive = true; */
    ...
    else
    {
        if (!followplayer)
        {
            ...
            if (buttons & JP_RIGHT)
            {
                ...
                ticbuttons[playernum] &= ~JP_RIGHT;
            }
            if (buttons & JP_LEFT)
            {
                m_paninc.x = -FTOM(F_PANINC);
                ticbuttons[playernum] &= ~JP_LEFT;
            }
            if (buttons & JP_UP)
            {
                m_paninc.y = FTOM(F_PANINC);
                ticbuttons[playernum] &= ~JP_UP;
            }
            if (buttons & JP_DOWN)
            {
                m_paninc.y = -FTOM(F_PANINC);
                ticbuttons[playernum] &= ~JP_DOWN;
            }
        }
        else
        {
        ...
        ticbuttons[playernum] &= ~JP_B;
        ticbuttons[playernum] &= ~JP_A;

        if ((buttons & (JP_3 | JP_NUM)) != 0)
        {
            if ((obuttons & (JP_3 | JP_NUM)) == 0
                && !viewactive)
            {
                AM_Stop();
                bigstate = false;
        ...
        if ((buttons & DAT_060010ec) != 0
            && (obuttons & DAT_060010ec) == 0)
        {
            grid = !grid;
        }
        ticbuttons[playernum] &= ~DAT_060010ec;

        if ((buttons & DAT_060010f4) != 0
            && (obuttons & DAT_060010f4) == 0)
        {
            bigstate = !bigstate;
            if (bigstate)
            {
        ...
        ticbuttons[playernum] &= ~DAT_060010f4;

        if ((buttons & DAT_060010f0) != 0
            && (obuttons & DAT_060010f0) == 0)
        {
            followplayer = !followplayer;
            f_oldloc.x = MAXINT;
        }
        ticbuttons[playernum] &= ~DAT_060010f0;
    }
